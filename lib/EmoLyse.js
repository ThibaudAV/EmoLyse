...
var EmoLyse = function(){
	...
	this.init = function() {
		var fs = require("fs");
		dir = __dirname+"/../"
		// si le dossier config n'existe pas
		if(!fs.existsSync(dir+"CONFIG")) {
			fs.mkdir(dir+"CONFIG", function(error) {
			  return false;
			});
		}
		// si le dossier de experience n'existe pas
		if(!fs.existsSync(dir+"EXPERIENCES")) {
			fs.mkdir(dir+"EXPERIENCES", function(error) {
			  return false;
			});
		}
	}
